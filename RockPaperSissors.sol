...
contract GameContract {
    ...
    function revealMove(uint256 moveId, string memory salt) external {
        ...
        if (gameRound.player1.playerAddress == msg.sender) {
            require(gameRound.player1.commitHash == hash, "Invalid hash.");

            gameRound.player1.figure = Figure(moveId);
            setStatus(Status.FirstPlayerRevealed);
        }
        else if (gameRound.player2.playerAddress == msg.sender) {
            require(gameRound.player2.commitHash == hash, "Invalid hash.");
    ...
    modifier gameContinuing() {
       require(gameRound.player1.figure == Figure.None || gameRound.player2.figure == Figure.None, "Round is continuing!");
        _;
    }
    ...
    function calculateWiner() gameContinuing external {
        ...
        else if ((gameRound.player1.figure == Figure.Rock && gameRound.player2.figure == Figure.Scissors) ||
            (gameRound.player1.figure == Figure.Paper && gameRound.player2.figure == Figure.Rock) ||
            (gameRound.player1.figure == Figure.Scissors && gameRound.player2.figure == Figure.Paper)) {
